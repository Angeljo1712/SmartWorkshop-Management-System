...
const saveBookingDetailsHandler = async (req, res) => {
  ...
  res.status(200).json({ ok: true, customerId: result.customerId, createdAccount: result.createdAccount });
};
...
const setDraftAccountPasswordHandler = async (req, res) => {
  const result = await bookingService.setDraftAccountPassword(req.body || {});
  res.status(200).json(result);
};
